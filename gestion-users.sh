...
if [ ! -f "$RUTORRENT"/ratxabox9.txt ]; then
	...
	"$CMDECHO" "cd ratxabox"
	"$CMDECHO" "chmod a+x bonobox.sh && ./bonobox.sh --fr"
	"$CMDECHO" ""
...
if [[ $VERSION = 11.* ]]; then
					#"$CMDAPTKEY" adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 3FA7E0328081BFF6A14DA29AA6A19B38D3D831EF
					"$CMDECHO" "deb http://download.mono-project.com/repo/debian bulleyes main" | "$CMDTEE" /etc/apt/sources.list.d/mono-official.list
